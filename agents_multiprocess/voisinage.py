...
def intra_route_swap(s,capa,max_capa):
...
def inter_route_swap(s,capa,max_capa):
    i = random.randint(0, len(s)-1)
    if len(s) <= 2:
             return(s)
    else:
...
def intra_route_shift(s,capa,max_capa):
    i=random.randint(0,len(s)-1)
    if len(s[i])<=2:
         return(s)
    else:
...
def inter_route_shift(s,capa,max_capa):
    i=random.randint(0,len(s)-1)
    if len(s)<=2:
             return(s)
    else:
...
def two_intra_route_swap(s,capa,max_capa):
    i=random.randint(0,len(s)-1)
    if len(s[i])<=2:
         return(s)
    else:
...
def two_intra_route_shift(s,capa,max_capa):
    i=random.randint(0,len(s)-1)
    if len(s[i])<=3:
         return(s)
    else:
        j=random.randint(0,len(s[i])-3)
        a=s[i][j]
        b=s[i][j+1]
        del s[i][j]
        del s[i][j+1]
...
def del_small_route(s,capa,max_capa):
    l=len(s)
    size_route=[]
    for i in range(l):
...
def del_random_route(s,capa,max_capa):
    l=len(s)
...
def test_capacity(sol, capacities, max_capacity):
    for route in sol:
...
def del_small_route_w_capacity(s,capa,max_capa):
    ...
    if (l != 1):
        ...
        vois = del_small_route(sp,capa,max_capa)
        for i in range(l):
            if test_capacity(vois,capa,max_capa):
                return(vois)
            else:
                sp = copy.deepcopy(S)
                vois = del_small_route(sp,capa,max_capa)
...
def del_random_route_w_capacity(s,capa,max_capa):
    ...
    if (l != 1):
        ...
        vois = del_random_route(sp,capa,max_capa)
        print(vois)
        for i in range(l):
            if test_capacity(vois,capa,max_capa):
                return(vois)
...
def inter_route_swap_w_capacity(s,capa,max_capa):
    l = len(s)
    S = copy.deepcopy(s)
    vois = inter_route_swap(s,capa,max_capa)
    print(vois)
    for i in range(l):
        if test_capacity(vois,capa,max_capa):
            return(vois)
...
def inter_route_shift_w_capacity(s,capa,max_capa):
    l = len(s)
    S = copy.deepcopy(s)
    vois = inter_route_shift(s,capa,max_capa)
    for i in range(l):
        if test_capacity(vois,capa,max_capa):
            return(vois)
        else:
